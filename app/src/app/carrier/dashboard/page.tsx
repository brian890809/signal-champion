...
import { getJobsByCarrierId, getAvailableJobs, getUserById } from '@/lib/store';
...
export default function CarrierDashboard() {
  ...
  useEffect(() => {
    ...
    // Fetch jobs for the carrier
    if (user) {
      const carrierJobs = getJobsByCarrierId(user.id);
      setMyJobs(carrierJobs);
      
      // Filter active and completed jobs
      setActiveJobs(carrierJobs.filter(job => 
        job.status === 'accepted' || job.status === 'in_transit'
      ));
      
      setCompletedJobs(carrierJobs.filter(job => 
          ...
          <div className="px-4 py-8 sm:px-0">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-5 w-0 flex-1">
                      <dl>
                        ...
                        <dd>
                          <div className="text-lg font-medium text-gray-900">{user?.name || 'Carrier'}</div>
                        </dd>
                      </dl>
                    </div>
                  </div>
              ...
              <div className="overflow-hidden rounded-lg bg-white shadow">
                ...
                <div className="bg-gray-50 px-5 py-3">
                  <div className="text-sm">
                    <Link href="/carrier/my-jobs" className="font-medium text-indigo-700 hover:text-indigo-900">
                      View my jobs
                    </Link>
                  </div>
                </div>
              ...
              <div className="overflow-hidden rounded-lg bg-white shadow">
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-5 w-0 flex-1">
                      <dl>
                        ...
                        <dd>
                          <div className="text-lg font-medium text-gray-900">{availableJobs.length}</div>
                        </dd>
                      </dl>
                    </div>
                  </div>
                ...
                <div className="bg-gray-50 px-5 py-3">
                  <div className="text-sm">
                    <Link href="/carrier/available-jobs" className="font-medium text-indigo-700 hover:text-indigo-900">
            ...
            <div className="mt-4 overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
              {availableJobs.length > 0 ? (
                ...
                <div className="text-center py-8 px-4 sm:px-6 lg:px-8">
                  <p className="text-sm text-gray-500">No available jobs found.</p>
                  <div className="mt-6">
                    <Link
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {activeJobs.map((job) => (
                      <tr key={job.id}>
                        ...
                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                          <Link href={`/carrier/jobs/${job.id}`} className="text-indigo-600 hover:text-indigo-900">
                            Update<span className="sr-only">, {job.id}</span>
                          </Link>
                        </td>
                      </tr>
                    ))}
